...
app.layout = html.Div(children=[
    ...
    html.Div(
        dcc.Tabs(
            ...
            dcc.Tab(
                label='Testing 2',
                value='return-distro-tab',
                children=[
                    dcc.Graph(
                        id='return-distro-chart'
                    )
                ]
            )
        ])
...
@app.callback(  
    [Output('quant-chart','figure'),
     ...
     Output('return-distro-chart', 'figure')],
    [Input('tab-view', 'value'),
...
def update_plot(tab                 : str,
    ...
    if tab == "candle-stick":
        ...
        return [fig, ticker_dict[ticker][1], go.Figure()]
    
    elif tab == 'return-distro-tab':
        
        sql_cmd =   f"""
                        SELECT  date_time, (open + close)/2.0 FROM {time_frame_table[time_frame]}
                        WHERE stock_id = {ticker_dict[ticker][0]}
                        ORDER BY date_time DESC
                        LIMIT {periods}
                    """
        ...
        data = { pd.Timestamp(p[0]) : {"avg" : p[1]} for p in rv}
        df = pd.DataFrame(data.values(), data.keys())
        df = df.pct_change()
        df.loc[:,'avg'] *= 100
        ...
        fig.add_traces(go.Histogram(x=df['avg'], nbinsx=100))
        
        return [go.Figure(), ticker_dict[ticker][1], fig]
